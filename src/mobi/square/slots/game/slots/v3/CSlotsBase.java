...
public abstract class CSlotsBase implements ISlotsConfig {
			...
			for (int j = 0; j < SYMBOLS_COUNT; j++) {
				...
				for (int k = 0; k < blank_symbols.length; k++) {
					...
					if (i > 0) {
						...
						// Top
						if (j > 0 && current[i - 1][j - 1].getSymbol() == blank_symbols[k]) continue;
						// Bottom
						if (j < SYMBOLS_COUNT - 1 && current[i - 1][j + 1].getSymbol() == blank_symbols[k]) continue;
					}
					// Right Side
					if (i < REELS_COUNT - 1) {
						// Middle
	...
	protected void readTable() throws IOException {
		...
		String line; int line_n = 0;
		line = reader.readLine(); line_n++;
