...
public class SlotsApi extends AndroidApi {
	...
	public void requestLogin(String token, LoginType type, String name, CancelableHandler response_handler) throws StringCodeException {
		...
		switch (type) {
			case OK:		params.put("oauth", "ok");	break;
			case FACEBOOK:	params.put("oauth", "fb");	break;
			case GOOGLE:	params.put("oauth", "gg");	break;
			default:		return;
		}
	...
	public List<SlotsInfo> responseSlotsList(String response) throws StringCodeException {
		...
		for (int i = 0; i < count; i++) {
			...
			info.setRequiresVip(slot.isNull("requires_vip") ? false : slot.getBoolean("requires_vip"));
			result.add(info);
		}
		return result;
	}
	...
	public boolean isMaxBet() {
		...
		if (this.current_bet < this.valid_bets.length - 1) return false;
		return true;
	}
	...
	private void setCanSpin(boolean can_spin) {
