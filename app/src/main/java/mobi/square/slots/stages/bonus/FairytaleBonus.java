...
import mobi.square.slots.ui.bonus.FairytaleBonusGroup;
import mobi.square.slots.ui.bonus.FairytaleBonusGroup.BarrelOpenedListener;
import mobi.square.slots.utils.json.JsonArray;
import mobi.square.slots.utils.json.JsonObject;

public class FairytaleBonus extends Header {

    private FairytaleBonusGroup game;
    ...
    private BarrelOpenedListener barrel_listener = new BarrelOpenedListener() {
        @Override
    ...
    public FairytaleBonus(SBasic parent) {
        super(parent);
        this.game = null;
    ...
    public void load(LoadingHandler handler) {
        super.load(false);
        Texture background = Connection.getManager().get("atlas/FairytaleBonusBg.jpg", Texture.class);
        TextureAtlas atlas = Connection.getManager().get("atlas/FairytaleBonus.pack", TextureAtlas.class);
        this.game = FairytaleBonusGroup.newInstance(background, atlas);
            ...
            this.timer.scheduleTask(new Task() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Connection.getInstance().requestInitSlots(SlotsType.FAIRYTALE, handler);
                        parent_screen.parent.showLoading(new SSinglePlayer(SlotsType.FAIRYTALE), handler);
                    } catch (StringCodeException e) {
                        Log.log(e);
                    }
                }
