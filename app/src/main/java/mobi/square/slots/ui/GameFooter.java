    ...
    private GameFooter(Basic parent, TextureAtlas atlas, TextureAtlas autospin_atlas) {
        ...
        this.spin_button.addListener(new ActorGestureListener() {
            ...
            public boolean longPress(Actor actor, float x, float y) {
                System.out.println("aki pendiente cuando es auto");
        ...
        this.spin_button.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if (!utils.gameUnlocked) {
                    if (utils.maxSpinToLock != 0 && utils.numberSpinDone >= utils.maxSpinToLock) {
                        utils.idPurchase = 1;
                        String msg = "Unlock the complete game for %";
                        Connection.getWrapper().showMessagePIXTEL(msg, 1);
                        return;
                    }
                    if (utils.maxBetToLock != 0 && utils.accumalatedBet >= utils.maxBetToLock) {
                        utils.idPurchase = 2;
                        String msg = "Unlock the complete game for %";
                        Connection.getWrapper().showMessagePIXTEL(msg, 2);
                        return;
                    }
                }
