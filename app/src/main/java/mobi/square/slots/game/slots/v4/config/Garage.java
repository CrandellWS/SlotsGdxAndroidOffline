...
import mobi.square.slots.game.slots.bonusgames.CGarageBoxes;
import mobi.square.slots.game.slots.bonusgames.CGarageLocks;
import mobi.square.slots.game.slots.v4.CSlotsBase;
import mobi.square.slots.utils.utils;

public class Garage extends CSlotsBase {
    ...
    private static final double zero_negative_percent = 0.35d;//0.2
    // Процент выпадения WILD'ов на выигрышной комбинации
    private static final double wild_percent = 0.15d;//
    ...
    private static final Symbol[] non_wild_symbols = {Symbol.SCATTER, Symbol.N06, Symbol.N07, Symbol.N08};
    // Возможные ставки
    ...
    private static final String title = "garage";
    private static final String background_path = "/SlotsWeb3/resources/img/slots/garage_bg.js";
    private static final String symbols_path = "/SlotsWeb3/resources/img/slots/garage_v3.js";
    private static final String background_hi_path = "/SlotsWeb3/resources/img/slots/garage_bg_hi.js";
    private static final String symbols_hi_path = "/SlotsWeb3/resources/img/slots/garage_hi_v3.js";
    ...
    public int checkPayout(int amount) {
        // N07 - box
        // N08 - lock
        int count_n07 = 0, count_n08 = 0;
        CSymbol[][] current = super.parent.getCurrentSymbols();
        for (int j = 0; j < current.length; j++) {
            CSymbol[] line = current[j];
            for (int i = 0; i < line.length; i++) {
                if (line[i].getSymbol() == Symbol.N07) {
                    count_n07++;
                } else if (line[i].getSymbol() == Symbol.N08) {
                    count_n08++;
                }
            }
        }
        if (count_n07 >= 3) {
            for (int j = 0; j < current.length; j++) {
                for (int i = 0; i < current[j].length; i++) {
                    if (current[j][i].getSymbol() == Symbol.N07) {
                        current[j][i].setSpecial(true);
                    }
                }
            }
            this.generateBonusGameBoxes();
        } else if (count_n08 >= 3) {
            for (int j = 0; j < current.length; j++) {
                for (int i = 0; i < current[j].length; i++) {
                    if (current[j][i].getSymbol() == Symbol.N08) {
                        current[j][i].setSpecial(true);
                    }
                }
            }
            this.generateBonusGameLocks();
        }
        return amount;
    }

    public void generateBonusGameLocks() {
        CGarageLocks game_locks = new CGarageLocks(super.parent);
        game_locks.generate();
        super.parent.setBonusGameInstance(game_locks);
    }

    public void generateBonusGameBoxes() {
        CGarageBoxes game_boxes = new CGarageBoxes(super.parent);
        game_boxes.generate(super.parent.getBet());
        super.parent.setBonusGameInstance(game_boxes);
    }
