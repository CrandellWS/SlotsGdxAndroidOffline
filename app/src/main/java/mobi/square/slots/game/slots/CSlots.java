...
public class CSlots implements Serializable {
    ...
    public static ISlotsConfig getConfig(CSlots slots, SlotsType type) {
        switch (type) {
            case FAIRYTALE:
                return new mobi.square.slots.game.slots.v3.config.Fairytale(slots);
            case UNDERWATER_LIFE:
                return new mobi.square.slots.game.slots.v3.config.UnderwaterLife(slots);
            case GLADIATORS:
                return new mobi.square.slots.game.slots.v4.config.Gladiators(slots);
            case CRAZY_MONKEY:
                return new mobi.square.slots.game.slots.v4.config.CrazyMonkey(slots);
            case GARAGE:
                return new mobi.square.slots.game.slots.v4.config.Garage(slots);
            case MONEY_GAME:
                return new mobi.square.slots.game.slots.v4.config.MoneyGame(slots);
            case RESIDENT:
                return new mobi.square.slots.game.slots.v4.config.Resident(slots);
            case BOOK_OF_RA:
    ...
    public void unpack(Map<String, String> strings, Map<String, Integer> integers) {
        ...
        this.risk_game = this.value(integers, "risk_game", this.risk_game ? 1 : 0) > 0;
    ...
    public boolean isCanBetMore() {
        ...
        for (int i = 0; i < this.getValidBets().length; i++) {
            if (this.getValidBets()[i] > this.getBet()) {
                return this.isMoneyEnough(this.getValidBets()[i], this.getLinesCount());
            }
        }
        return false;
    }
    ...
    public boolean isCanBetLess() {
        if (this.isFreeSpin()) return false;
        if (this.isBonusGame()) return false;
        return this.getBet() > this.getValidBets()[0];
    }
    ...
    public boolean isCanLinesMore() {
        ...
        for (int i = 0; i < this.getValidLines().length; i++) {
            if (this.getValidLines()[i] > this.getLinesCount()) {
                return this.isMoneyEnough(this.getBet(), this.getValidLines()[i]);
            }
        }
        return false;
    }
    ...
    public boolean isCanLinesLess() {
        if (this.isFreeSpin()) return false;
        if (this.isBonusGame()) return false;
        return this.getLinesCount() > this.getValidLines()[0];
    }
    ...
    public boolean isMoneyEnough() {
        ...
        int user_bet = this.getBet() * this.getLinesCount();
        return this.getParent().getMoney() >= user_bet;
    }
